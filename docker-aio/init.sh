...
wget -q https://github.com/DEvmIb/telerising-helper/blob/main/docker-aio/health-check.sh -O /tmp/health-check.sh
chmod +x /tmp/health-check.sh

if [ "$HEALTH_DISABLE" == "" ]; then screen -dmS healt-check bash -c 'while :; do date +%s > /tmp/health-check.up; /tmp/health-check.sh >> /tmp/health-check.log 2>&1; sleep 10; done'; fi
